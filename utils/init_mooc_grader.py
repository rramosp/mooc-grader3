import sys, urllib.request, urllib.parse, urllib.error, re, datetime, ntplib, pytz, tzlocal, traceback, httplib2, gspread
import numpy as np
import pandas as pd
...
def compute_grades(sheet_name, gc=None):
    ...
    print("Processing", sheet_name, end=' ')
...
def check_result(result,pid):
    ...
    if type(maxgrade)==float and (result<0 or result>maxgrade):
...
if sys.argv[1]=="SUBMIT_SOLUTION":
   ...
   with open("utils/student_function/"+grader_fname.decode().split()[0], 'r') as myfile:
